...
from modules.system.schemas import user as UserSchema
...
async def user_query(
    query: UserSchema.UserQuery, _: dict = Depends(check_permissions)
) -> Response[user_list_schema]:
...
async def user_create(
    data: UserSchema.UserAdd, _: dict = Depends(check_permissions)
) -> Response[UserSchema.UserRead]:
    return await UserService.create_item(data)


@router.delete("/{pk}", summary="用户删除")
async def user_delete(pk: int, _: dict = Depends(check_permissions)) -> Response:
...
async def user_update(
    pk: int, data: UserSchema.UserPut, _: dict = Depends(check_permissions)
) -> Response:
